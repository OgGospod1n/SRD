<div class="float-left w-full mb-10">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4" wire:loading.remove>
        ...
        <div id="html5menu-{{ $document["document_id"] ? $document["document_id"] : $document["id"] }}" style="display:none" class="showcase">
            <command label="Редактировать" icon="edit" onclick="window.location.href='{{ route('showDocument', $document["document_id"] ? $document["document_id"] : $document["id"]) }}'">
            <command label="Поделиться" onclick="window.location.href='{{ route('shareDocument', $document["document_id"] ? $document["document_id"] : $document["id"]) }}'"> 
            <command label="Сохранить в формате PDF" onclick="window.location.href='{{ route('showDocumentAsPdf', $document["document_id"] ? $document["document_id"] : $document["id"]) }}'">
            <hr>
            <command label="Удалить" icon="delete" onclick="deleteFiles('{{ $document["document_id"] ? $document["document_id"] : $document["id"] }}', '{{ $document["name"] }}', '{{ csrf_token() }}')">
        </div>
        @endforeach
    </div>
</div>
